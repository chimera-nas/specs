...
def load_cell_config(cell):
    """The cell config, with // comments stripped (the configs carry them)."""
    path = os.path.join(CONFIG_DIR, f"{cell}.json")
    with open(path) as fh:
        return json.loads(re.sub(r"^\s*//.*$", "", fh.read(), flags=re.M))


def check_config_profile(cell, measured):
    """Compare a live probe against the knobs the cell config pins.

    The config is not documentation: the corpus is GENERATED from it, so every
    trace already encodes these answers and a drifted knob means the whole
    corpus describes a filesystem that is not the one under test.  Knobs left
    on "explore" are not pinned and so cannot drift.
    """
    cfg = load_cell_config(cell)
    drift = []
    for group in ("features", "policies"):
        for key, want in (cfg.get(group) or {}).items():
            if want == "explore":
                continue
...
class Replayer:
    ...
    def op_open(self, pid, rv, res, fs):
        ...
        if self.check_status(res["e"], r["err"]) and res["e"] == 0:
            ...
            #
            # Closing is not free, though.  close(2) releases every byte-range
            # lock the calling process holds on that file, and the model --
            # whose open failed -- still believes it holds them: the next
            # process to ask for those bytes is granted them, and a later
            # blocking acquire waits on a lock the model cannot see.  Hold the
            # descriptor to the end of the trace when the model has this pid
            # holding locks, and close it in cleanup() with the rest.
            if self.pid_holds_lock(pid):
                self.strays.append((pid, r["ret"]))
            else:
    ...
    def op_lseek(self, pid, rv, res, fs):
        ...
        # Whether any byte the two sides actually DISAGREE about was ever
        # WRITTEN, as opposed to merely reserved by fallocate or spanned by a
        # truncate.  ext4 cannot tell an allocated-but-unwritten extent from a
        # hole (see EXT4-5), and the shadow is the only thing that knows -- but
        # the question has to be asked of the disputed range, not of the whole
        # file from the queried offset.  A file written at 0..4096 and then
        # fallocated at 5120 has written bytes after a query at 1024 and still
        # disagrees only about the reservation: the model rounds the partial
        # block up to data and answers EOF, ext4 keeps byte-granular extents
        # and answers 4096.  An ENXIO on either side disputes everything up to
        # the end of the shadow.
        self._ctx["written_disputed"] = self.written_between(
            ino,
            res["off"] if res["e"] == 0 else -1,
            r["ret"] if r["err"] == 0 else -1)
    ...
    def pid_holds_lock(self, pid):
        """True when the model has `pid` holding at least one byte-range lock
        on any file.  ps.locks is keyed by the (pid, ino) pair; an entry whose
        byte map has been emptied by an unlock does not count."""
        ps = self._ctx.get("ps")
        if not ps:
            return False
        return any(bytes_ for (p, _), bytes_ in ps.get("locks", {}).items()
                   if p == pid and bytes_)

    def written_between(self, ino, a, b):
        """True when the byte shadow holds a written (nonzero) byte anywhere
        in the range the two sides disagree about.  A zero byte is either a
        hole or a fallocate-reserved block; either way nothing put data there,
        so the disagreement is over how to classify a reservation.  Either
        bound may be -1, meaning the side answered ENXIO and so disputes
        everything to the end of the shadow."""
        if ino is None:
            return False
        sh = self.shadow.get(ino)
        if not sh:
            return False
        lo = min(x for x in (a, b) if x >= 0) if (a >= 0 or b >= 0) else 0
        hi = len(sh) if (a < 0 or b < 0) else max(a, b)
        lo, hi = max(lo, 0), min(hi, len(sh))
        return hi > lo and any(sh[lo:hi])
    ...
    def op_pwritev(self, pid, rv, res, fs):
        ...
        self._check_write(r, res)
        # pwritev takes the offset the same way pwrite does, and Linux ignores
        # it through an O_APPEND descriptor the same way too.  Without this the
        # append is silently absorbed and the file is one write too long: the
        # divergence then surfaces at whatever later read runs off the end,
        # naming a count mismatch instead of the write that caused it.
        self._check_pwrite_landed(pid, rv, res, fs)
        return r
...
# so the pinned EXT4_PROFILE above -- the baseline ext4_deviations.py is
# written against -- is a measurement rather than a guess.
# --------------------------------------------------------------------------
...
def main():
    ...
    if args.probe or args.check_profile:
        ...
        if args.check_profile:
            drift = check_config_profile(args.target, measured)
            if drift:
                print("\nThe cell config no longer describes this "
                      "filesystem:", file=sys.stderr)
                for line in drift:
                    print(f"  {line}", file=sys.stderr)
                print("\nThe corpus is generated FROM those knobs, so a "
                      "drifted config means every trace was built for a "
                      "filesystem that is not the one under test.  Re-measure "
                      "and update harness/posix/configs/<cell>.json.",
                      file=sys.stderr)
                return 1
            print("\nconfig matches the live filesystem")
        return 0
