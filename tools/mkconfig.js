...
const cells = []
// CMake quoted strings interpret backslashes as escapes. Use forward-slash
// paths on Windows, and protect characters that have CMake list/string syntax.
...
cmakeLines.unshift(`set(SPECS_${spec.family}_GENSPEC "${cmakePath(path.join(spec.stage, `${spec.family}.gen.json`))}")`)
cmakeLines.unshift(`set(SPECS_${spec.family}_DEPS "${[...allDeps].map(cmakePath).join(';')}")`)
cmakeLines.unshift(`set(SPECS_${spec.family}_ALL_TRACES "${allTraces.map(cmakePath).join(';')}")`)
